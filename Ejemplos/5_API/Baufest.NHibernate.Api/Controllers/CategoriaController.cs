...
namespace Baufest.NHibernate.Api.Controllers
{
    public class CategoriaController : ApiController
    {
        ...
        public IEnumerable<Categoria> Get([FromUri]string nombre = null)
        {
            ...
            {
                var nombreCategoria = nombre ?? string.Empty;
                var categorias = session
                    .CreateSQLQuery(
                       @"SELECT cat.IdCategoria, cat.Nombre
                            FROM Categoria cat 
                            WHERE cat.Nombre LIKE :nombreCategoria")
                    .AddEntity(typeof(Categoria))
